...
class QwenVLProcessor(ProcessorMixin):
    ...
    def decode(self, pred: Union[List, paddle.Tensor], skip_special_tokens: bool = False):
        """
        This method forwards all its arguments to PreTrainedTokenizer's [`~PreTrainedTokenizer.decode`]. Please refer
        to the docstring of this method for more information.
        """

        return self.tokenizer.decode(pred.cpu(), skip_special_tokens=skip_special_tokens)
    ...
    def __call__(self, image_paths: List[str]):
        ...
        for image_path in image_paths:
            if image_path.startswith("http://") or image_path.startswith("https://"):
                ...
                if "/root/images/" in image_path:
                    image_path = os.path.join("/tmp/v2_mnt/HCG/huangxiaoshuang/llavamed_images/images", name)
                else:
                    image_path = os.path.join('/tmp/v2_mnt/HCG/huangxiaoshuang/SAMed2D_v1/images', name)
                if not os.path.exists(image_path):
                    print('image do not exist: ',image_path)
